...
def on_click(event, matrix, playerValue, fig, ax, returnPos):
    if event.inaxes == ax:
        ...
        if matrix[row, col] == 0:
            ...
            flatIndex = row * matrix.shape[1] + col
            print(f"Clicked position: ({row}, {col}), flat index: {flatIndex}, set to {playerValue}")
            returnPos.append(flatIndex)
            plt.close(fig)

def update_matrix(fig, ax, matrix):
    ax.clear()
    colors = {0: 'white', 1: 'green', -1: 'red'}
    cmap = mcolors.ListedColormap([colors[x] for x in sorted(colors.keys())])
    norm = mcolors.BoundaryNorm(sorted(colors.keys()) + [max(colors.keys()) + 1], cmap.N)
...
def display_colored_matrix(matrix, playerValue, gameStatus):
    fig, ax = plt.subplots()
    returnPos = []
    
    if gameStatus == 0:
        fig.canvas.mpl_connect('button_press_event', lambda event: on_click(event, matrix, playerValue, fig, ax, returnPos))
    
    update_matrix(fig, ax, matrix)
    plt.show()
    
    return returnPos[0] if returnPos else None
...
if whoStarts == 0:
    ...
    while flagGame == 0:
        ...
        if gameStatus == 1:
            clickedLocation = display_colored_matrix(np.reshape(board, (boardDimension, boardDimension)), -1, 1)
            print("AI Wins")
            break
        elif gameStatus == -1:
            clickedLocation = display_colored_matrix(np.reshape(board, (boardDimension, boardDimension)), -1, -1)
        ...
        # Player 2
        playerTurn = 2
        clickedLocation = display_colored_matrix(np.reshape(board, (boardDimension, boardDimension)), -1, 0)
        availableLocationsBoolean = board == 0
        availableLocations = baseIndexArray[0][availableLocationsBoolean[0]]
